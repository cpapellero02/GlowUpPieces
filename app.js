...
let cart = [], currentPD = null, pdQty = 1, pickedStars = 0;
  ...
  document.getElementById('productGrid').innerHTML = list.map(p => `
    <article class="p-card" onclick="openPD(${p.id})">
      ...
      <div class="p-body">
        ...
        <div class="p-foot">
          <div class="p-price">${p.oldPrice?`<span class="old-p">₱${p.oldPrice}</span>`:''}₱${p.price}</div>
          <button class="add-btn" onclick="event.stopPropagation();addToCart(${p.id})"><i class="fas fa-plus"></i></button>
        </div>
      </div>
    </article>
  `).join('');
  ...
  document.getElementById('pdInner').innerHTML = `
    <div class="pd-inner">
      ...
      <div class="pd-body">
        ...
        <button class="pd-add" onclick="addToCartQty(${p.id})"><i class="fas fa-shopping-bag"></i> Add to Bag</button>
      </div>
    </div>
  `;
...
function renderCartItems() {
  ...
  box.innerHTML = cart.map(item => `
    <div class="ci">
      ...
      <div class="ci-info"><h5>${item.name}</h5><p>₱${item.price}</p></div>
      <div class="ci-qty">
...
function removeCI(id) { cart = cart.filter(i => i.id !== id); updateCartBadge(); renderCartItems(); }
...
function goPayment() {
  if (!document.getElementById('chkName').value.trim() || !document.getElementById('chkAddress').value.trim()) {
    toast('Please fill in your shipping details.'); return;
  }
...
function trackOrder() {
  ...
  if (!order) { res.innerHTML = `<p class="tracker-note">Order not found. Try: GUP-2024-001</p>`; return; }
  const icons = ['fa-receipt','fa-cog','fa-box','fa-truck','fa-home'];
  res.innerHTML = `
    <div class="t-steps">${order.steps.map((label,i) => `
      <div class="t-step ${i<order.step?'done':''} ${i===order.step?'active':''}">
...
function renderInventory() {
  document.getElementById('inventoryBody').innerHTML = INVENTORY.map(item => {
    ...
    return `<tr>
      ...
      <td><code style="font-size:.84rem;color:var(--muted)">${item.sku}</code></td>
      <td>${item.cat}</td>
      <td>₱${item.price}</td>
      <td>${item.stock||'—'}</td>
...
function setupStarPicker() {
  ...
  document.getElementById('starPicker').addEventListener('mouseleave', () => {
    document.querySelectorAll('#starPicker span').forEach(x => x.classList.toggle('lit', +x.dataset.v <= pickedStars));
  });
}
...
function handleSearch(e) {
  ...
  const hits = PRODUCTS.filter(p => p.name.toLowerCase().includes(q) || p.cat.toLowerCase().includes(q) || p.desc.toLowerCase().includes(q));
  box.innerHTML = hits.length
    ? hits.map(p => `<div class="sr-item" onclick="searchPick(${p.id})"><strong>${p.name}</strong> — ₱${p.price}</div>`).join('')
    : '<p style="color:var(--muted);font-size:1rem">No products found.</p>';
}
